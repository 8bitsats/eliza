...
def generate_trading_decision(
    ...
    template = ChatPromptTemplate.from_messages(
        [
            (
              "system",
              """You are a visionary portfolio manager inspired by Steve Jobs and Gordon Gekko, managing a hedge fund with a focus on AI and Web 3. 
            - Prioritize innovative opportunities (e.g., tech, crypto, blockchain) with high growth potential.
            - Take aggressive positions when confidence is high (>75%), but remain cautious otherwise.
            - Optimize for long-term value and market disruption, integrating traditional and Web 3 markets.
            - Follow trading rules: buy/sell within cash limits, short/cover within margin, respect max_shares."""
        ),
        (
            "human",
            """Make trading decisions based on these signals:
            ...
            {{
              "decisions": {{
                "TICKER": {{"action": "buy/sell/short/cover/hold", "quantity": int, "confidence": float, "reasoning": "string"}}
              }}
            }}"""
        ),
    ])
