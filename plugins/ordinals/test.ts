import { ordinalsPlugin } from './ordinals-plugin.js';
...
async function testOrdinalsPlugin() {
  try {
    ...
    const ordinalsAPI = new (MOCK_MODE ? MockOrdinalsAPI : OrdinalsAPI)(
      process.env.HIRO_API_KEY || 'mock-api-key',
      'http://localhost:3000',
      true
    );

    const ordinalsUtils = new (MOCK_MODE ? MockOrdinalsUtils : OrdinalsUtils)(
      process.env.ORD_PATH || '/mock/ord/path',
      process.env.BITCOIN_CONF_PATH || '/mock/bitcoin.conf'
    );
