...
export interface RareSatoshiHunterOptions {
  rarity?: string[];
  limit?: number;
  offset?: number;
  minValue?: number;
  ...
  sortBy?: 'value' | 'rarity' | 'age';
  sortDirection?: 'asc' | 'desc';
}

export interface RareSatoshi {
  ordinal: string;
  rarity: string;
...
export interface MarketStats {
  ...
  byRarity: {
    mythic: RarityStat;
    legendary: RarityStat;
    epic: RarityStat;
    rare: RarityStat;
    uncommon: RarityStat;
    common: RarityStat;
  };
  inscribed: { count: number; percentage: number };
  uninscribed: { count: number; percentage: number };
...
export class RareSatoshiHunter {
  private ordinalsAPI: OrdinalsAPI;
  private ordinalsUtils: OrdinalsUtils;
   ...
   */
  async findRareSatoshis(options: RareSatoshiHunterOptions = {}): Promise<RareSatoshi[]> {
    ...
    try {
      ...
      try {
        for (const rarity of rarities) {
          ...
          const parsedSatoshis: LocalRareSatoshi[] = this.parseSatoshiOutput(rawOutput, rarity);
          
          rareSatoshis = [...rareSatoshis, ...parsedSatoshis.map((sat: LocalRareSatoshi) => ({
            ordinal: sat.ordinal,
            rarity: sat.rarity,
      ...
      rareSatoshis.sort((a, b) => {
        ...
        if (sortBy === 'rarity') {
          const aRarityValue = rarityOrder[a.rarity as keyof typeof rarityOrder] || 0;
          const bRarityValue = rarityOrder[b.rarity as keyof typeof rarityOrder] || 0;
   ...
   */
  private parseSatoshiOutput(output: string, rarity: string): LocalRareSatoshi[] {
    try {
   ...
   */
  async getSatoshiDetails(ordinal: string): Promise<any> {
    try {
   ...
   */
  async getMarketStats(rarity: string = 'all'): Promise<MarketStats> {
    try {
      ...
      for (const satoshi of satoshis) {
        const rarityType = satoshi.rarity as keyof typeof stats.byRarity;
