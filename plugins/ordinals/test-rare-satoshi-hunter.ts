...
const mockRuntime = {
  getSecret: (key: string) => {
    if (key === 'HIRO_API_KEY') return process.env.HIRO_API_KEY || '';
    if (key === 'UNISAT_API_KEY') return process.env.UNISAT_API_KEY || '';
    if (key === 'ORDISCAN_API_KEY') return process.env.ORDISCAN_API_KEY || '';
    if (key === 'BLOCK_CYPHER_API_TOKEN') return process.env.BLOCK_CYPHER_API_TOKEN || '';
    return '';
  },
