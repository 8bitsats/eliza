...
export class OrdinalsAPI {
  ...
  private useLocalApiFirst: boolean;
   ...
   */
  constructor(hiroApiKey: string, localApiUrl?: string, useLocalApiFirst = true) {
   ...
   */
  async getSatoshi(ordinal: string) {
   ...
   * @returns Promise with the inscriptions data
   */
  async getSatoshiInscriptions(ordinal: string, options: { limit?: number; offset?: number } = {}) {
    const { limit = 20, offset = 0 } = options;
   ...
   */
  async getInscriptionTransfers(id: string, options: { limit?: number; offset?: number } = {}) {
    const { limit = 20, offset = 0 } = options;
   ...
   */
  private async makeRequest(endpoint: string, isContent = false) {
    ...
    for (const api of apis) {
      try {
        ...
        const options = {
          headers,
          ...(isContent ? { responseType: 'arraybuffer' as const } : {})
        };
